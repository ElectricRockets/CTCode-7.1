...
public class DistanceSensorEstimator {
    ...
    public DistanceSensorEstimator(Telemetry telemetry, Supplier<Pose2d> robotPoseSupplier, Pose2d sensorPose, DistanceSensor sensor, double[] wallOffsets) {
    ...
    public void update() {
        ...
        Pose2d globalSensorPose = new Pose2d(
                ...
                robotPose.getHeading() + sensorPose.getHeading()
        );
        ...
        Pose2d robotPoseEstimateError = new Pose2d( (distanceReading - estimatedIntersectDistance) * Math.sin(estimatedIntersectAngle), 0, estimatedCorrectionAngle);

        robotPoseError = new Pose2d(
                Math.cos(estimatedCorrectionAngle) * robotPoseEstimateError.getX(),
                Math.cos(estimatedCorrectionAngle) * robotPoseEstimateError.getY(),
                0
        );
    ...
    public Pose2d getUpdatedRobotPoseError() {
        update();
        return robotPoseError;
    }
